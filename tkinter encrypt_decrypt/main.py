...
# Save the user input as new key
...
def openfile():
    cwd = os.getcwd()
    selectfile = filedialog.askopenfilename(initialdir=cwd, title="Select a file")
...
def encrypt():
    ...
    with open("encryptionkey.key", "rb") as key_for_encrypt:
        ...
        key_for_encrypt.close()
        fernet_e = Fernet(e_key)
        encrypted_file = fernet_e.encrypt(original)
        with open("Encrypted_File.txt", "wb") as encrypted_message:
